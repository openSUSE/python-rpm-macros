...
#### Multi flavor Requirements definition

There are some macros ready to define requirements of python-flavored
packages, so when the __`%python_subpackages`__ macro expands and
generate one package for each python flavor, the requirement and other
dependency definitions are modified accordingly.

* __BuildRequires__: Use the __`%python_module`__ macro here, for
  example `BuildRequires: %{python_module setuptools}` will expand to
  `BuildRequires: python311-setuptools`, `BuildRequires: python312-setuptools`,
  and so on, so all flavored python-package will be included during
  build.
* __Requires, Provides and similar__: __Don't use `%python_module`__
  in this case.

  Python package names in `Requires`, `Requires(pre)` and all other
  scriptlet runtime requirements, `Provides`, `Recommends`, `Suggests`,
  `Obsoletes`, `Conflicts`, `Supplements` and `Enhances` are automatically
  converted to the correct flavor in the subpackage declarations
  autogenerated by `%python_subpackages`.

  If the requirement or capability name starts with `python-`, with the
  default python flavor (`python313-` in Tumbleweed, `python3-` in older
  distributions), or with the same python prefix as your package name
  (so `python3-bar` in package `python3-foo`), then the python name is
  changed to match that of the generated package. This also works for
  `python` itself.

* __%requires_eq and %requires_ge__: Use `%python_requires_eq` and
  `%python_requires_ge` instead. These macros will be translated to
  the corresponding one without the `python_` prefix for each
  subpackage. For example you can declare
  `%python_requires_eq python-requests` and that will expand to:
  - `%requires_eq python311-requests` for 311 flavor subpackage
  - `%requires_eq python312-requests` for 312 flavor subpackage
  - `%requires_eq python313-requests` for 313 flavor subpackage
  - ...
